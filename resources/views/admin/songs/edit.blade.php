...
<div class="container-fluid py-5 px-lg-5 bg-main min-vh-100 text-light">
    ...
    <form action="{{ route('admin.songs.update', $song) }}" method="POST">
        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-md-3">
                <label for="youtube_url" class="form-label small text-secondary text-uppercase fw-bold">Link YouTube</label>
                <input type="text" name="youtube_url" id="youtube_url" class="form-control bg-dark text-light border-secondary" value="{{ old('youtube_url', $song->youtube_url) }}" placeholder="Incolla URL video">
            </div>
            ...
            <div class="col-md-1">
                ...
                <input type="number" name="position" id="position" class="form-control bg-dark text-light border-secondary" value="{{ old('position', $song->position) }}" min="1">
            </div>
            ...
            <div class="col-md-2">
                ...
                <select name="edition_id" id="edition_id" class="form-select bg-dark text-light border-secondary" required>
                    @foreach ($editions as $edition)
                        <option value="{{ $edition->id }}" {{ old('edition_id', $song->edition_id) == $edition->id ? 'selected' : '' }}>
            ...
            <div class="col-lg-3">
                ...
                <select name="artists[]" id="artists" class="form-select bg-dark text-light border-secondary" multiple style="height: 500px;">
                    @foreach ($artists as $artist)
                        <option value="{{ $artist->id }}" {{ $song->artists->contains($artist->id) ? 'selected' : '' }}>
                            {{ $artist->name }}
                        </option>
                    @endforeach
                </select>
                <div class="form-text text-secondary mt-2 small">
                    <i class="fa-solid fa-circle-info me-1"></i> Ctrl per multi-selezione.
                </div>
            </div>
            ...
            <div class="col-lg-3">
                ...
                <select name="awards[]" id="awards" class="form-select bg-dark text-light border-secondary" multiple style="height: 500px;">
                    @foreach ($awards as $award)
                        <option value="{{ $award->id }}" {{ $song->awards->contains($award->id) ? 'selected' : '' }}>
                            {{ $award->name }}
                        </option>
                    @endforeach
                </select>
