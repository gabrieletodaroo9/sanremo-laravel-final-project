        ...
        <form action="{{ route('admin.songs.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <div class="form-text text-secondary mt-2">
                    <i class="fa-solid fa-circle-info me-1"></i> Tieni premuto <strong>Ctrl</strong> (o <strong>Cmd</strong>) per selezionare più artisti (es. duetti).
                </div>
            </div>

            <div class="mb-5">
                <label for="lyrics" class="form-label small text-secondary text-uppercase fw-bold">Testo della Canzone</label>
                <textarea name="lyrics" id="lyrics" rows="6" class="form-control bg-dark text-light border-secondary" placeholder="Inserisci qui il testo del brano..."></textarea>
            </div>

            <div class="d-flex justify-content-end gap-3 pt-4 border-top border-secondary">
                <button type="submit" class="btn btn-secondary px-5 text-uppercase fw-bold">Salva Canzone</button>
            </div>
        </form>
    </div>
</div>
