            ...
            @foreach ($editions as $edition)
                <div class="accordion-item bg-transparent border-bottom border-secondary-subtle ">
                    <h2 class="accordion-header">
                        <button class="accordion-button collapsed bg-transparent text-light px-0 py-2 shadow-none"
                            type="button" data-bs-toggle="collapse" data-bs-target="#collapseEdition{{ $edition->id }}">
                            <div class="d-flex align-items-center">
                                <div class="rounded-circle bg-dark border border-secondary me-3 d-flex align-items-center justify-content-center"
                                ...
                                <div>
                                    ...
                                    <small class="d-block text-secondary text-lowercase">{{ $edition->songs->count() }}
                                        brani registrati</small>
                                </div>
                            </div>
                        </button>
                    </h2>

                    <div id="collapseEdition{{ $edition->id }}" class="accordion-collapse collapse"
                        data-bs-parent="#accordionEditions">
                                        ...
                                        @foreach ($edition->songs->sortBy('position') as $song)
                                            <tr class="border-bottom border-secondary-subtle">
                                                <td class="py-1 bg-transparent">
                                                    <span
                                                        class="fw-bold {{ $song->position <= 3 && $song->position > 0 ? 'text-warning' : 'text-secondary' }}">
